...
// 			if err != nil {
// 				log.Printf("(%v) fail to acquire lock for type (%v): %v.\n", accountNumber, Type, err.Error())
// 				return "", err
// 			}
// 			log.Printf("Account Number (%v) entered the critical section for (%v).\n", accountNumber, Type)
// 			msg, err := p.work(data) // ignore any failure occurs in this stage temporaily.
// 			err_ := p.dl.Release(aNo, Type)
// 			if err_ != nil {
// 				log.Printf("(%v) fail to release lock of type  %v: %v.\n", accountNumber, Type, err.Error())
// 				return "", err_
// 			}
